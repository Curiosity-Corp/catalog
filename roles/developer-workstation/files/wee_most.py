...
def create_server_option_cb(data, config_file, section, option_name, value):
    if not re.match(r'^[a-z]+\.(command_2fa|password|url|username)$', option_name):
...
class ChannelBase:
    def __init__(self, server, **kwargs):
        ...
        self.is_unread_marker_set = False
...
def hydrate_channel_posts_cb(buffer, command, rc, out, err):
    ...
    for post_id in reversed(response["order"]):
        ...
        if not channel.is_unread_marker_set and not builded_post.read:
            weechat.buffer_set(channel.buffer, "unread", "")
            channel.is_unread_marker_set = True
...
def create_channel_from_channel_data(channel_data, server):
    if channel_data["type"] == "D":
        match = re.match(r"(\w+)__(\w+)", channel_data["name"])
...
def buffer_switch_cb(data, signal, buffer):
    for server in servers.values():
        previous_channel = server.get_channel_from_buffer(EVENTROUTER.previous_buffer)
        if previous_channel:
            previous_channel.mark_as_read()
            break
    ...
    EVENTROUTER.previous_buffer = buffer
...
class Server:
    def __init__(self, id):
        ...
        if not self.url or not self.password:
            raise ValueError("Server {} is not fully configured (need at least url and password/token)".format(id))

        # If no username, treat password as a personal access token (skip login)
        self.use_token_auth = not self.username
...
def connect_server_cb(server_id, command, rc, out, err):
    ...
    token_search = re.search(r"[tT]oken: (\w*)", out)
...
def run_get_me_for_token_auth(server, cb, cb_data):
    url = server.url + "/api/v4/users/me"
...
def connect_server_token_auth_cb(server_id, command, rc, out, err):
    server = servers[server_id]

    if rc != 0:
        server.print_error("Token auth failed — is the personal access token valid?")
        return weechat.WEECHAT_RC_ERROR

    try:
        response = json.loads(out)
    except (json.JSONDecodeError, ValueError) as e:
        server.print_error("Token auth parse error: {}".format(str(e)))
        return weechat.WEECHAT_RC_ERROR
    ...
    try:
        ...
        server.print_error("WebSocket connection failed: {}".format(str(e)))
        return weechat.WEECHAT_RC_ERROR
...
def connect_server(server_id):
    ...
    if server.use_token_auth:
        # Skip login — use PAT directly as Bearer token
        server.token = server.password
        server.print("Using personal access token (no login required)")
        EVENTROUTER.enqueue_request(
            "run_get_me_for_token_auth",
            server, "connect_server_token_auth_cb", server.id
        )
    else:
        EVENTROUTER.enqueue_request(
            "run_user_login",
