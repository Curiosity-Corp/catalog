...
def main() -> None:
    ...
    for fragment in (
        ...
        "workstation_release_discovery_available: false",
        "Safe package and configuration",
    ):
        if fragment not in latest_tasks + main_tasks:
    ...
    if 'no_log: "{{ github_api_token | default(\'\') | length > 0 }}"' not in latest_tasks:
        raise SystemExit("authenticated GitHub release queries must not log their headers")
